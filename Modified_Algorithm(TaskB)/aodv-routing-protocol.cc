...
RoutingProtocol::RoutingProtocol ()
  : m_rreqRetries (2),
    ...
    m_queue (m_maxQueueLen, m_maxQueueTime),//change
    m_requestId (0),
    m_seqNo (0),
...
RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::aodv::RoutingProtocol")
    ...
    .AddAttribute ("MaxQueueLen", "Maximum number of packets that we allow a routing protocol to buffer.",
                   ...
                   MakeUintegerChecker<uint32_t> ())//max number of packet komale queue jam toiri kora possible
    .AddAttribute ("MaxQueueTime", "Maximum time packets can be queued (in seconds)",
                   TimeValue (Seconds (30)),
                   MakeTimeAccessor (&RoutingProtocol::SetMaxQueueTime,
                                     &RoutingProtocol::GetMaxQueueTime),
                   MakeTimeChecker ())//etao komate hobe.jate taratari drop hoy
...
RoutingProtocol::PrintRoutingTable (Ptr<OutputStreamWrapper> stream, Time::Unit unit)const
{
  *stream->GetStream () << "Node: " << m_ipv4->GetObject<Node> ()->GetId ()
                        ...
                        << ", Queue_Size: " << m_queue.GetMaxQueueLen()
                                      ...
                                      UnicastForwardCallback ucb, ErrorCallback ecb)
{
  ...
  bool result = m_queue.Enqueue (newEntry);//change 
  //u_int32_t size =m_queue.GetSize();

 //cout<<"enqueue er porer queue  aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa: "<<size<<endl;
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      rreqHeader.SetForwardBufferCount(0);
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  //uint32_t queue_size = m_queue.GetSize();
  //cout<<"queue size of the rreq line 1:"<<queue_size<<endl;
  //cout<<" size line 2: "<<m_queue.getsize()<<endl;
  //m_queue.GetMaxQueueLen();
  uint32_t threshold = m_queue.GetMaxQueueLen()*0.75;
  //cout<<"threshold :"<<threshold<<endl;
  //uint8_t for_buffer = rreqHeader.GetForwardBufferCount();
  ...
  //Increment RREQ forward BUFFER count
  //uint8_t size = queue_size+ rreqHeader.GetForwardBufferCount();
  // if(queue_size>for_buffer){
  //     rreqHeader.SetForwardBufferCount(size);
  // }
  ...
  uint32_t size = m_queue.GetSize();
  if (IsMyOwnAddress (rreqHeader.GetDst ()))
    {
      rreqHeader.SetForwardBufferCount(size);
      m_routingTable.LookupRoute (origin, toOrigin);
      NS_LOG_DEBUG ("Send reply since I am the destination");
      SendReply (rreqHeader, toOrigin);//nijei dst
      return;
    }
  /*
          ...
          if (!rreqHeader.GetDestinationOnly () && toDst.GetFlag () == VALID)
            {
              m_routingTable.LookupRoute (origin, toOrigin);
              rreqHeader.SetForwardBufferCount(size);
              SendReplyByIntermediateNode (toDst, toOrigin, rreqHeader.GetGratuitousRrep ());//info ache dst somporke
              return;
            }
  ...
  {   
      //cout<<"queue_size is greater than 48.it is returning."<<endl;
      //cout<<"queue_size : "<<queue_size<<endl;
      return;
  }
  else if(queue_size<threshold){
      //cout<<"forward buffer set hocche "<<endl; 
      rreqHeader.SetForwardBufferCount(queue_size);
      //cout<<"queue_size is not greater than 48.and the rreq is forwarding"<<endl;
      //cout<<"QUEUE_SIZE : "<<queue_size<<endl;
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
...
RoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
  ...
  RrepHeader rrepHeader ( /*prefixSize=*/ 0,0, /*hops=*/ 0, /*dst=*/ rreqHeader.GetDst (),
...
{
  NS_LOG_FUNCTION (this);
  RrepHeader rrepHeader (/*prefix size=*/ 0,m_queue.GetSize(), /*hops=*/ toDst.GetHop (), /*dst=*/ toDst.GetDestination (), /*dst seqno=*/ toDst.GetSeqNo (),
  ...
  if (gratRep)
    {
      RrepHeader gratRepHeader (/*prefix size=*/ 0,m_queue.GetSize(), /*hops=*/ toOrigin.GetHop (), /*dst=*/ toOrigin.GetDestination (),
...
RoutingProtocol::RecvReply (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
  ...
  uint32_t size_queue = m_queue.GetSize();
  uint32_t threshold = m_queue.GetMaxQueueLen()*0.75;

  if(size_queue>threshold){
  
  //cout<<"rrep is forwarding"<<endl;
  rrepHeader.SetbackwardBufferCount(size_queue);
  Ptr<Packet> packet = Create<Packet> ();
  SocketIpTtlTag ttl;
  ttl.SetTtl (tag.GetTtl () - 1);
  packet->AddPacketTag (ttl);
  ...
  else if(size_queue<threshold){
    //cout<<"rrep is NOT forwarding"<<endl;
    return;
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      Ptr<Socket> socket = j->first;
      Ipv4InterfaceAddress iface = j->second;
      RrepHeader helloHeader (/*prefix size=*/ 0,0, /*hops=*/ 0, /*dst=*/ iface.GetLocal (), /*dst seqno=*/ m_seqNo,
